/**
 * ╔══════════════════════════════════════════════════════════════════╗
 * ║  THE BILLY LIVING — Client Database Layer                        ║
 * ║  READ-ONLY. No write(), delete() methods exist in this file.    ║
 * ╚══════════════════════════════════════════════════════════════════╝
 *
 * MODE A — localStorage (demo/dev): works instantly, no Firebase needed.
 * MODE B — Firebase Firestore (production): fill in firebase-config.js
 *
 * To activate Firebase: set USE_FIREBASE = true below.
 */
...
(function(window) {
  ...
  // ── localStorage implementation (READ-ONLY API) ───────────────────
  let _cache = null;
  const _listeners = {};
  ...
  window.addEventListener('storage', e => {
    if (e.key === STORE_KEY) {
      _cache = null;
      Object.keys(_listeners).forEach(col => _notify(col));
    }
  });

  // ── Firebase implementation (READ-ONLY API) ───────────────────────
  let _fb = null;
  async function _initFirebase() {
    ...
    try {
      ...
      const { getFirestore, collection, doc, onSnapshot, getDoc, getDocs } = await import('https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js');
      const { getAuth, onAuthStateChanged } = await import('https://www.gstatic.com/firebasejs/10.12.0/firebase-auth.js');
      const app = getApps().length ? getApps()[0] : initializeApp(cfg);
      _fb = { db: getFirestore(app), auth: getAuth(app), collection, doc, onSnapshot, getDoc, getDocs };
      return _fb;
    } catch(e) { console.warn('Firebase init failed, using localStorage:', e); return null; }
  }

  // ── PUBLIC READ-ONLY API ──────────────────────────────────────────
  window.TBL_DB = {

    /** Read a collection or meta once */
    async read(col) {
      if (USE_FIREBASE) {
    ...
    async listen(col, callback) {
      if (USE_FIREBASE) {
        ...
        if (fb) {
          if (col === 'meta') {
            return fb.onSnapshot(fb.doc(fb.db, 'meta', 'global'), snap => {
              callback(snap.exists() ? snap.data() : {});
            });
          }
          return fb.onSnapshot(fb.collection(fb.db, col), snap => {
            callback(snap.docs.map(d => ({ id: d.id, ...d.data() })));
          });
        }
      }
      // localStorage: register listener and fire immediately
  ...
  // ── NOTE: write() and delete() do NOT exist in this file ─────────
